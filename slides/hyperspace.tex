...
\begin{frame}{Outline}
  ...
  \item 1. \emph{Accursed} Geometry
  \item 2. Lifting the Curse...
  \item 3. An Activity
  \end{itemize}
\end{frame}
...
%% ``The trend today is towards more observations but even more so, \alert{to
%%       radically larger numbers of variables} – voracious, automatic, systematic
%%     collection of hyper-informative detail about each observed instance.''
...
\begin{frame}{}
  ...
  \only<-5>{%
    \begin{equation*} \begin{aligned}
        (1 - \alert<3>{\epsilon})\alert<2>{\text{Distance}} &\leq %
          \alert<4,5>{\text{Projected Distance}} \\
        &\, \\
        \alert<4,5>{\text{Projected Distance}} &\leq %
          (1 + \alert<3>{\epsilon})\alert<2>{\text{Distance}}\\
    \end{aligned} \end{equation*}
  }%
  \only<6>{%
    \begin{equation*}
      (1 - \epsilon)\|\vu - \vv\| \leq \|P_{\alert{k}}(\vu) - P_{\alert{k}}(\vv)\| \leq %
      (1 + \epsilon)\|\vu - \vv\|
    \end{equation*}
  }
  % Annotation
  \visible<2-5>{%
    \begin{textblock}{3}(+1.5,-6.0)
      {\textblockcolor{}
        \centering%
